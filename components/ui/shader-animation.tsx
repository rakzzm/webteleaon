...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    for(float i = 0.0; i < 4.0; i++) {
      ...
      d = pow(abs(0.005 / max(abs(d), 0.0001)), 1.5);
...
export default function ShaderAnimation({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const rect = host.getBoundingClientRect();
      mouseRef.current.x = (event.clientX - rect.left) / Math.max(rect.width, 1);
      mouseRef.current.y = 1 - (event.clientY - rect.top) / Math.max(rect.height, 1);
    };
  ...
  return (
    <div className={cn("shader-container", backgroundOnly && "shader-container--background", className)}>
      <canvas ref={canvasRef} className={cn("shader-canvas", canvasClassName)} />

      {showControls ? (
        <div className="controls">
          <button className="interactive-toggle" onClick={() => setIsInteractive((current) => !current)}>
