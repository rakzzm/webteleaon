...
class SmokeRenderer {
  ...
  private color: [number, number, number] = [0.16, 0.78, 0.91];
  ...
  render(now = 0) {
    ...
    gl.uniform2f(program.resolution ?? null, canvas.width, canvas.height);
    gl.uniform1f(program.time ?? null, now * 0.001);
    gl.uniform3fv(program.uColor ?? null, this.color);
...
export function SmokeBackground({ smokeColor = "#28c7e8", className }: SmokeBackgroundProps) {
  ...
  useEffect(() => {
    ...
    try {
      renderer = new SmokeRenderer(canvas);
    } catch (error) {
      console.error("Smoke background failed to initialize", error);
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 72% 14%, rgba(40,199,232,0.28), transparent 32%), radial-gradient(circle at 18% 24%, rgba(224,0,131,0.18), transparent 30%), linear-gradient(135deg, #ffffff 0%, #effcff 48%, #fff1fa 100%)",
          opacity: 0,
